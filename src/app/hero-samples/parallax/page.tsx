  ...
  return (
    <HeroSampleShell title="패럴랙스 3D">
      ...
      >
        ...
        >
          <div
            className="absolute -top-24 left-[10%] h-72 w-72 rounded-full bg-[var(--neon-blue)]/20 blur-[90px] transition-transform duration-300 ease-out"
            style={{ transform: `translate(${offset.x * -40}px, ${offset.y * -40}px)` }}
          />
          <div
            className="absolute right-[8%] top-[30%] h-64 w-64 rounded-full bg-[var(--neon-pink)]/15 blur-[80px] transition-transform duration-300 ease-out"
            style={{ transform: `translate(${offset.x * -60}px, ${offset.y * -60}px)` }}
          />
          <span
            className="animate-float-note absolute left-[12%] top-[20%] hidden text-3xl text-[var(--neon-blue)]/40 transition-transform duration-300 ease-out md:block"
            style={{ transform: `translate(${offset.x * 30}px, ${offset.y * 30}px)` }}
          >
            ♪
          </span>
          <span
            className="animate-float-note-slow absolute right-[14%] top-[55%] hidden text-3xl text-[var(--neon-pink)]/35 transition-transform duration-300 ease-out md:block"
